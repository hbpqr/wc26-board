window.DASH_DATA = {
  ...
  "daily": [
   ...
   },
   {
    ...
    "records": [
     ...
     "📊 周五1-4场全终：无持仓·pool 278.55不变。065-066(11:00)待开·请主会话生成065-066 slip"
    ]
   }
  ],
  "evLedger": [
 ...
 "todayMatches": [
  ...
  },
  {
   ...
   "note": "埃及vs伊朗(G组末轮·埃及4分领先·平局即以组首晋级)。埃及Hamdi Fathi肌肉伤缺·Abdel-Meguid头伤规程；伊朗Cheshmi全程缺席。埃及需赢才锁G1·伊朗需赢出线·中等动机差。模型集成主胜38.9%·平局概率36.8%均衡。"
  },
  {
   "num": "周五066",
   "round": "周五",
   ...
   "note": "新西兰vs比利时(G组末轮·双方均0胜2平积2分·胜者大概率晋级)。比利时Doku回归·Ngoy停赛·Debast存疑；新西兰无伤停完整阵容。比利时强力热门但连平无状态·新西兰G组末轮奇迹存在。模型集成客胜81.8%。"
  }
 ],
 "holdings": {
  "invested": 100,
 ...
 "training": {
  ...
  "headline": "已喂 64 场预测 · 集成 Brier 0.512 vs 市场去水 0.5233（略胜）· 只比随机0.667好一点（地板线,非alpha）",
  "lesson": "**28场复盘硬结论(诚实版):** 集成 Brier 0.512 **仍没跑赢市场去水(0.5233)**——模型暂无可证明的边。它只比'瞎猜'(随机0.667)好一点,那是地板不是本事。平局占36%(异常高)模型从不押平→10个自动判错卡死命中率;回测所有补救都≤57%,平局事前分不出来。**边在纪律和情报、不在模型方向**(可迁股市:高效市场里纯模型必收敛到市场)。⚠️别把'跑赢随机'当 alpha——真市场CLV还在攒(竞彩收盘价未留存),没有真CLV前'有没有边'仍是自说自话。",
  "graduation": "完成训练 = 喂满50场 + Brier稳定且**真CLV转正** + 权重收敛。当前 64/50,权重已收敛(A市场去水重仓),但**Brier尚未跑赢市场、CLV待积累**——离'有边'还差关键一步:留存赛前收盘价算真CLV。"
 }
};