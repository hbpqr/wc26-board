window.DASH_DATA = {
 ...
 "training": {
  ...
  "lesson": "**28场复盘的硬结论(2026-06-19):** 平局占36%(异常高),模型从不押平→10个自动判错=命中率被压在57%的天花板;回测所有补救都≤57%,平局事前分不出来。**模型已到顶,调参榨不出边——边只在信息差+纪律。** 让胜/大热门让球是数据证明的陷阱(36%被逼平:西班牙0:0/葡萄牙1:1/卡塔尔…)。这条直接迁股市:高效市场里模型必收敛到市场、赢不了。",
  "graduation": "完成训练 = 喂满 50 场 + Brier 稳定 < 市场 + 权重收敛。当前 28/50(56%),权重已基本收敛(A 重仓),还差 22 场达毕业线 → 然后引擎迁股市。",
  "retro": {
   "title": "28场预测 vs 真实 · 复盘发现(2026-06-19)",
   "headline": "平局盲点:36%的比赛是平局,模型押平0次→10个自动判错(占全部错误的83%)",
   "tested": "回测了所有补救(加权平局/只在势均力敌加/削热门顶),无一招能超过57%——平局事前真分不出来",
   "conclusion": "模型方向命中已到顶(≈市场),调参榨不出边。边只在①信息差②纪律。让胜/大热门让球=数据证明的陷阱(36%被逼平)。此结论可直接迁股市。"
  }
 },
 "calibration": {
  "stats": {
