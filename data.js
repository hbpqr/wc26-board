window.DASH_DATA = {
 ...
 "slip": {
  "title": "7月6日 · 全盘扫描空仓 · 21:00 最终版",
  "status": "empty",
  "detail": "betRound=周一(093葡萄牙vs西班牙 07-07 03:00·094美国vs比利时 07-07 08:00,均在售未开打);额外提前核查周二(095阿根廷vs埃及 07-08 00:00·096瑞士vs哥伦比亚 07-08 04:00)。终盘重算+全盘口扫描如下:【093】HAD 主胜@3.78值0.89/平@3.30值0.88/客胜@1.78值0.88 全<1.0;让球(+1) 让胜@1.81值0.89(edge+0.2pp)/让平值0.80(edge-2.7pp)/让负值0.98(edge+2.5pp)均远低于5pp门槛;比分0:0值1.22但按§5禁猜比分票;情报侧西班牙双翼Nico Williams(腹股沟)/Yeremy Pino(肩伤)缺战已连续多日多源确认、早已price in,无新增量。【094】HAD 主胜@2.10值0.87/平@3.40值0.90/客胜@2.79值0.90 全<1.0;让球(+1)三项edge均<1.5pp;情报侧比利时对Balogun解禁裁决的正式上诉已被FIFA受理、裁决未公布(存在赛前反转可能),但方向不确定(不知道上诉成败),按§17.2'这是真被低估还是我在给中间结论找证据'自检,无法据此形成有方向的edge,不构成下注理由;Debast腿伤doubtful已price in。【095】HAD elig.had.single=0(胜平负仅能串关,不可单买),且三项值本身也全<1.0(主0.89/平0.88/客0.85);让球(-1)三项edge均<1.5pp;fade-favorite协议触发检查:集成主胜(阿根廷)72.5%≥60%入场门槛,但竞彩平局@4.65 vs 国际共识(Polymarket 20.2%)公平赔率4.95 → CLV=-6.1%(负,且比历次最小容忍值088的-2.8%更差),按协议'负CLV一律不下'的硬边标准不触发,且该场HAD本身单关不可购买,双重排除。埃及后防三缺(法图赫/阿卜杜勒蒙纳姆/哈菲兹)、萨拉赫预计出战均已连续多源确认无新增量。【096】HAD 主胜@2.98值0.87/平@2.85值0.88/客胜@2.26值0.90 全<1.0;让球(+1)三项edge均<1.5pp;情报侧哥伦比亚科尔多瓦报销+J罗因病存疑(金特罗或顶替)已入库,尝试用--tilt 1.0,0.93(哥伦比亚进攻-7%)重跑Poisson验证,但当前C-Poisson模型集成权重仅0.003%(weights.json,历史表现差已被自动降权),tilt对集成概率无实质影响(主29.3/平31.0/客39.7不变),说明此情报暂无法转化为可执行edge,非隐瞒而是模型结构限制,如实记录。四场综合:HAD全部<1.0、让球edge全部<5pp门槛(最高2.9pp)、比分票违反§5禁令、fade-favorite在唯一触发场次(095)因负CLV+单关不可购双重卡死,亦评估任何2串1组合(如093让负×096让胜)仍是零/负edge腿相乘、不创造真实价值,予以否决。按§0'只当某场真找不到方向才空、且只空那场'的carve-out,本轮4场逐一扫描后均无站得住方向,非偷懒的整轮'市场高效'借口。诚实提醒:这是连续第32轮无票(hourly巡检持续追踪),反映16强淘汰赛热门场次竞彩定价效率高,非系统故障;影子book四场预测均已记录在案,免费学习照常进行。资金池=cashNow250.55+pendingStake0=250.55,本轮空仓不变。",
  "items": [],
  "summary": {
   "stake": 0,
   "evLow": 0,
  ...
  "matrix": [],
  "copyText": "老板你好，竞彩足球，7月6日：\n\n明晚(093葡萄牙vs西班牙、094美国vs比利时)两场都查过了，赔率都没给出足够便宜的价，暂不下单，空仓。\n\n谢谢！",
  "reviewNote": "✓ 已独立复核 20:41 ｜ 草稿合规,无删改(逐场重算predictions_log.json的ens×odds:093 h0.89/d0.88/a0.88、094 h0.87/d0.90/a0.90、095 h0.89/d0.88/a0.85、096 h0.87/d0.88/a0.90,全<1.0与草稿一致;095的odds_data.js had.elig.single=0已核实只能串关;095 fade-favorite CLV核算=(4.65-4.95)/4.95=-6.1%<0未触发,已见fade_favorite_forward.json对应not_placed_neg_clv记录;items为空,summary/matrix/copyText/stake均为0,一致无需改动)"
 },
 "calibration": {
  "stats": {
   "direction": "3/5",
